...
public:
	...
	// Public interface
	bool recompile();
	void use() const { glUseProgram(shaderProgram_id); }
	void DeleteShaderProgram() {glDeleteProgram(shaderProgram_id);}

	void friend attach(ShaderProgram& sp, Shader& s);
	unsigned int GetShaderProgramID(){return shaderProgram_id;};

private:
	unsigned int shaderProgram_id; // the id of the shader program
